...
const generateRandomId = () => {
  ...
  return randomChars
    .split("")
    .map(() =>
      randomChars.at(Math.round(Math.random() * (randomChars.length - 1))),
    )
    .join("")
    .substring(0, 20);
};
...
class CustomExtensionModal extends React.Component {
  ...
  getExtensionURL() {
    ...
    if (this.state.type === "file") {
      return new Promise((resolve, reject) => {
        ...
        reader.onerror = () =>
          reject(
            new Error(`Could not read extension as data URL: ${reader.error}`),
          );
  ...
  async handleLoadExtension() {
    ...
    if (this.props.swapId) {
      ...
      if (
        !confirm(
          "Failure to swap extensions will cause the extension to be flatout removed, are you sure the inputed extension has matching id's and has no errors?",
        )
      ) {
        return;
      }
  ...
  fetchSwapUrl() {
    return (
      this.props.vm.extensionManager.extensionUrlFromId(this.props.swapId) ?? ""
    );
  }
  fetchSwapText() {
    return (
      this.props.vm.extensionManager.extUrlCodes[this.fetchSwapUrl()] ?? ""
    );
  }
  render() {
    return (
      <CustomExtensionModalComponent
        ...
        onChangeUnsandboxed={
          this.canChangeUnsandboxed() ? this.handleChangeUnsandboxed : null
        }
