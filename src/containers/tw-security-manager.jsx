...
const isTrustedExtension = (url) =>
  isTrustedExtensionOrigin(url) || extensionsTrustedByUser.has(url);
...
class TWSecurityManagerComponent extends React.Component {
  constructor(props) {
    super(props);
    bindAll(this, ["handleAllowed", "handleDenied", "projectWillChange"]);
  ...
  async canUnsandbox(name) {
    ...
    const { showModal } = await this.acquireModalLock();
    const allowedUnsandbox = await showModal(SecurityModals.Unsandbox, {
      name: name || "",
    });
    if (!allowedUnsandbox) {
